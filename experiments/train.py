...
# forward one sample
def train(input_variables, cap_target_variables, encoder, word_decoder, encoder_optimizer,
    ...
    def _one_pass(input_variable, cap_target_variable):

        _im_embedding= encoder.feature(input_variable)  # [1, HiddenSize]

        _cap_len = cap_target_variable.size()[0]
        #_word_num = 0
        ...
        word_decoder_hidden = _im_embedding.view(1, 1, -1) #sent_topics[sent_i]  # init RNN using topic vector
        ...
        return _cap_loss, _word_seen_num
    ...
    for i in range(batch_size):
        ...
        cur_cap_loss, cur_word_num = _one_pass(input_variable,cap_target_variable)
    ...
    loss = cap_loss #config.CapLoss_Weight * cap_loss + config.StopLoss_Weight * stop_loss
    loss.backward()

    encoder_optimizer.step()
    #sent_decoder_optimizer.step()
    word_decoder_optimizer.step()
    return cap_loss.data[0] / total_word_num
...
def train_iters(model, train_dataset, val_dataset, config, start_iter=1):
    ...
    for iter in range(int(start_iter), n_iters+int(start_iter)):
        ...
        while dataset_index + batch_size < dataset_size:
            ...
            caption_loss = train(input_variables, cap_target_variables, encoder, word_decoder,encoder_optimizer,
                                            word_decoder_optimizer, criterion, config)
            ...
            if dataset_index % print_every == 0:
                ...
                print('[Iter: %d, Batch: %d]%s (%d %d%%) caption_loss = %.3f; '
                      'Val: bleu_score = [%.3f, %.3f, %.3f, %.3f]' %
                    (iter, batch_index, time_since(start, dataset_index / dataset_size), dataset_index,
                     dataset_index / dataset_size * 100, print_caption_loss_avg, bleu_scores[0],
                     bleu_scores[1], bleu_scores[2], bleu_scores[3]))
        ...
        val_bleu_scores = evaluate_pairs(model, train_dataset.lang, val_dataset, config, np.load)
        print("[Iter {}] Validation BLEU: {:.3f} {:.3f} {:.3f} {:.3f}".format(iter,
