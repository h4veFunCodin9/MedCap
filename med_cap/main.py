...
def stat_captions(pairs, mode='word'):
    ...
    for path, caption in pairs:
        ...
        for sent in caption:
            if mode == 'word':
                import fool
                #cur_len = len([term.strip() for term in fool.cut(sent)[0] if len(term.strip())]) TODO
...
class Encoder(torch.nn.Module):
    def __init__(self, config):
        ...
        self.vgg = M.vgg11(pretrained=False) # TODO
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description="Medical Captioning")  #TODO
    ...
    random.shuffle(train_pairs)
    stat_captions(train_pairs, mode=args.seg_mode)

    class BRATS_Config(Config):
        ...
        def __int__(self):
            super(BRATS_Config, self).__init__()
